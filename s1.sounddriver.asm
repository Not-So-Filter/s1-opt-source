...
.skipdelay:
		tst.b	(f_palmode).w
		beq.s	.noupdate
		cmpi.b	#5,SMPS_RAM.v_dbl_update_counter(a6)
		bne.s	.noupdate
		clr.b	SMPS_RAM.v_dbl_update_counter(a6)
		bra.w	UpdateSMPS.dblupdate

.noupdate:
		addq.b	#1,SMPS_RAM.v_dbl_update_counter(a6)
		rts
...
; loc_71E4A:
FMSetRest:
		bset	#1,SMPS_Track.PlaybackControl(a5)	; set track to resting
...
.sfx_notRing:
		...
		bne.s	loc_722C6.locret		; Return if not
		st.b	SMPS_RAM.f_push_playing(a6)	; Mark it as playing
; Sound_notA7:
.sfx_notPush:
...
.sfxinitpsg:
		lsr.b	#4,d3
		movea.w	SFX_BGMChannelRAM(pc,d3.w),a5
		bset	#2,SMPS_Track.PlaybackControl(a5)	; Mark music track as being overridden
		move.b	d4,d0
...
.psgloop:
		...
		bsr.w	PSGNoteOff
		add.b	d6,SMPS_Track.Volume(a5)		; Apply current volume fade-in
		cmpi.b	#$E0,SMPS_Track.VoiceControl(a5)	; is this the Noise Channel?
		bne.s	.nextpsg				; no - skip
		move.b	SMPS_Track.PSGNoise(a5),(psg_input).l	; restore Noise setting
; loc_72B78:
.nextpsg:
		lea	SMPS_Track.len(a5),a5
		dbf	d7,.psgloop
